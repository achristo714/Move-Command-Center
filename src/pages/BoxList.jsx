    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`p-2 rounded-lg transition-colors ${selectMode ? 'bg-blue-50 text-blue-500' : 'text-slate-400'}`}
          >
            <CheckSquare className="w-5 h-5" />
          </button>
          <button
            onClick={() => setFilterOpen(!filterOpen)}
            className={`p-2 rounded-lg transition-colors ${filterOpen ? 'bg-blue-50 text-blue-500' : 'text-slate-400'}`}
          >
          ...
          <div className="flex gap-1">
            {STATUS_OPTIONS.map(s => (
              <button
                ...
                style={{ backgroundColor: s.value === 'packed' ? '#3b82f6' : s.value === 'loaded' ? '#f97316' : s.value === 'in_storage' ? '#8b5cf6' : s.value === 'delivered' ? '#f59e0b' : '#22c55e' }}
              >
                {s.label}
              </button>
            ))}
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="font-medium text-slate-600">No boxes yet</h3>
          <p className="text-sm text-slate-400 mt-1">Let's get packing!</p>
          <button
            onClick={() => navigate('/boxes/new')}
            className="mt-4 bg-blue-500 text-white rounded-lg px-4 py-2 text-sm font-medium"
          >
            Add your first box
          </button>
        </div>
